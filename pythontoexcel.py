...
write_sheet = wb['Second Sheet'] # Must match the name we created earlier

read_wb = xl.load_workbook('ProduceReport.xlsx') #Name of workbook
read_ws = read_wb['ProduceReport'] #Name of sheet 
...
for row in read_ws.iter_rows(min_row=2,max_row=maxR,max_col=maxC):
    ...
    #Increments current row otherwise it will just write to the same row
    current_row +=1
